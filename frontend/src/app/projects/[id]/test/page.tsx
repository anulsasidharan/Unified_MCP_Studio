...
export default function TestingConsolePage() {
  ...
  useEffect(() => {
    ...
    if (!token) { setError("Sign in to run tests."); return; }
    (async () => {
      try {
  ...
  async function startSandbox(e: FormEvent) {
    ...
    try {
      const res = await apiJson<{ session_id: string }>("/api/v1/testing/sandbox/start", {
        method: "POST", token, body: JSON.stringify({ project_id: projectId }),
      });
      setSessionId(res.session_id);
      setOutput(`Session started: ${res.session_id}`);
    } catch {
  ...
  async function runTool(e: FormEvent) {
    e.preventDefault();
    if (!sessionId) { setError("Start the sandbox first."); return; }
    const token = getStoredAccessToken();
    if (!token) return;
    ...
    setError(null);
    try {
      const res = await apiJson<{ ok: boolean; stdout: string; stderr: string; detail: string | null }>(
        "/api/v1/testing/sandbox/execute",
        { method: "POST", token, body: JSON.stringify({ session_id: sessionId, tool_name: toolName, arguments: parsed }) }
      );
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-3xl flex-col gap-6 px-6 py-12">
      ...
      <div className="flex gap-3 text-sm">
        <Link className="text-zinc-400 hover:text-white" href={`/projects/${projectId}/design`}>Designer</Link>
        <Link className="text-zinc-400 hover:text-white" href={`/projects/${projectId}`}>Overview</Link>
      </div>
      ...
      <form onSubmit={startSandbox} className="flex flex-wrap items-end gap-3">
        ...
        {sessionId && <span className="text-xs text-zinc-500">session: {sessionId}</span>}
      </form>
      ...
      <form onSubmit={runTool} className="flex flex-col gap-3 rounded-lg border border-zinc-800 p-4">
        ...
        <button type="submit" className="rounded-md border border-zinc-700 px-4 py-2 text-sm text-zinc-100">
          Execute tool
        </button>
      </form>
      ...
      <section>
        ...
        <pre className="mt-2 overflow-x-auto rounded-md bg-zinc-950 p-4 text-xs text-zinc-200">{output || "—"}</pre>
      </section>
    </main>
  );
}
